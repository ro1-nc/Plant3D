    ...
    public class SupportC
    {
        ...
        public void ReadSupportData()
        {
            ...
            if (AcadDoc != null)
            {
                ...
                List<TagWithLocation> TagLocation = new List<TagWithLocation>();
                                                    ...
                                                    if (ConcreteSupport.Count > 0)
                                                    {
                                                        for (int i = 0; i < ConcreteSupport.Count; i++)
                                                        {
                                                            if (ConcreteSupport[i].specData.SupportFileName.Equals(AcadDoc.Name))
                                                            {
                                                                ...
                                                                acText.TextString = count < 9 ? "PS-0" + (count).ToString() : "PS-" + (count).ToString();
                                                                ...
                                                                TagLocation.Add(new TagWithLocation(acText.Position, true));
                                                                //ConcreteSupport.RemoveAt(i);
                                                                count++;
                                                            }
                ...
                foreach (var SData in ListCentalSuppoData)
                {
                    if (SData.SupportType != null)
                    {
                        foreach (var SType in SData.ListSecondrySuppo)
                        {
                            ...
                            /*  else if (SType.Size != null && SType.Size.ToLower().Contains("angle"))
                              {
        ...
        public void CreateFullBlock(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, ref double tracex, double boxlen, double boxht, ref double spaceY, Document Document2D, string SupType, [Optional] int i)
        {
            ...
            else
            {
                ...
                if (spaceY > boxht)
                {
                    ProjectTo2D(AcadBlockTableRecord, AcadTransaction, AcadDatabase, ref tracex, boxlen, boxht, ref spaceY, Document2D, ListCentalSuppoData[i].SupportType, i);
                }
                else
                {
        ...
        public void ProjectTo2D(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, ref double tracex, double boxlen, double boxht, ref double spaceY, Document Document2D, string SupType, [Optional] int i)
        {
            ...
            foreach (SupporSpecData sec in ListCentalSuppoData[i].ListSecondrySuppo)
            {
                ...
                if (sec.SupportName.ToLower().Contains("plate") || sec.IsGussetplate || sec.IsAnchor)
                {
                    continue;
                }
            ...
            bool ChecfFor3rdDim = CheckVectorsArePlaner(allsecpart);
                    ...
                    if (primorder.First().ProjectionPlane != null && primorder.First().ProjectionPlane.Normal != null)
                    {
                        ...
                        foreach (SupporSpecData SecData in ListCentalSuppoData[i].ListPrimarySuppo)
                        {
                            ...
                            foreach (BoundingBoxFace FaceLoc in SecData.PrimBoundingBoxData)
                            {
                                ...
                                if (Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, GetVect3DFromPt3D(primorder.First().ProjectionPlane.Normal), new Vector3D(0, 0, 1)))) == 0)
                                {
                        ...
                        {
                            SecondaryDimension(AcadBlockTableRecord, AcadTransaction, AcadDatabase, secondary_sort, SecPart2D, AllParts, i, PrimPart2D);
                        }
                ...
                else if (ListCentalSuppoData[i].ListSecondrySuppo.Count > 0)
                {
                    ...
                    if (primorder.First().ProjectionPlane != null && primorder.First().ProjectionPlane.Normal != null)
                    {
                        ...
                        foreach (var ent in AllLiness)
                        {
                            ...
                            if (ent.IDSegment.ToLower().Contains("c"))
                            {
                                LineDraw(AcadBlockTableRecord, AcadTransaction, AcadDatabase, new Point3d(line.P1.X, line.P1.Y, line.P1.Z), new Point3d(line.P2.X, line.P2.Y, line.P2.Z), MyCol.Cyan);
                            }
                            else if (ent.IDSegment.ToLower().Contains("s"))
                            {
                                LineDraw(AcadBlockTableRecord, AcadTransaction, AcadDatabase, new Point3d(line.P1.X, line.P1.Y, line.P1.Z), new Point3d(line.P2.X, line.P2.Y, line.P2.Z), MyCol.Red);
                            }
                            else
                            {
                                LineDraw(AcadBlockTableRecord, AcadTransaction, AcadDatabase, new Point3d(line.P1.X, line.P1.Y, line.P1.Z), new Point3d(line.P2.X, line.P2.Y, line.P2.Z), MyCol.Blue);
                            }
                        ...
                        if (secondary_sort.Count() > 0)
                        {
                            SecondaryDimension(AcadBlockTableRecord, AcadTransaction, AcadDatabase, secondary_sort, SecPart2D, AllParts, i);
                        }
            ...
            else
            {
                ...
                if (ListCentalSuppoData[i].ListPrimarySuppo.Count > 0)
                {
                    ...
                    if (primorder.First().ProjectionPlane != null && primorder.First().ProjectionPlane.Normal != null)
                    {
                        ...
                        foreach (SupporSpecData SecData in ListCentalSuppoData[i].ListPrimarySuppo)
                        {
                            ...
                            foreach (BoundingBoxFace FaceLoc in SecData.PrimBoundingBoxData)
                            {
                                normal = GetVect3DFromPt3D(primorder.First().ProjectionPlane.Normal);
                ...
                var NEWWVC = Vector3D.CrossProduct(normal, new Vector3D(0, 0, 1));
                if (ListCentalSuppoData[i].ListPrimarySuppo.Count > 0)
                {
                    var primorder = ListCentalSuppoData[i].ListPrimarySuppo.OrderByDescending(e => e.Centroid.Z);
                    if (NEWWVC != null)
                    {
                        ...
                        foreach (SupporSpecData SecData in ListCentalSuppoData[i].ListPrimarySuppo)
                        {
                            ...
                            foreach (BoundingBoxFace FaceLoc in SecData.PrimBoundingBoxData)
                            {
                                ...
                                if (Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, NEWWVC, new Vector3D(0, 0, 1)))) > 0 && Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, NEWWVC, new Vector3D(0, 0, 1)))) < 89)
                                {
                            ...
                            try
                            {
                                ...
                                for (int j = 0; j < FacestoProj.Count; j++)
                                {
                                    ...
                                    for (int k = 0; k < FacestoProj[j].ListAllEdges.Count; k++)
                                    {
                                        ...
                                        GeometRi.Plane3d GeomPlaneRef = new GeometRi.Plane3d(GetGeometRiPt3DFromPt3D(ListCentalSuppoData[i].ListPrimarySuppo[0].ProjectionPlane.PointOnPlane), new GeometRi.Vector3d( NEWWVC.X, NEWWVC.Y, NEWWVC.Z));
                        ...
                        foreach (SupporSpecData SecData in ListCentalSuppoData[i].ListSecondrySuppo)
                        {
                            ...
                            foreach (FaceData FaceLoc in SecData.ListfaceData)
                            {
                                ...
                                if (Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(GetVect3dFromVect3D(FaceLoc.FaceNormal), NEWWVC, new Vector3D(0, 0, 1)))) > 0 && Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(GetVect3dFromVect3D(FaceLoc.FaceNormal), NEWWVC, new Vector3D(0, 0, 1)))) < 89 || HasOpenCircularEdge(FaceLoc))
                                {
                                    FacestoProj.Add(FaceLoc);
                            ...
                            try
                            {
                                ...
                                for (int j = 0; j < FacestoProj.Count; j++)
                                {
                                    ...
                                    for (int k = 0; k < FacestoProj[j].ListAllEdges.Count; k++)
                                    {
                                        ...
                                        GeometRi.Plane3d GeomPlaneRef = new GeometRi.Plane3d(GetGeometRiPt3DFromPt3D(primorder.First().ProjectionPlane.PointOnPlane), new GeometRi.Vector3d(NEWWVC.X, NEWWVC.Y, NEWWVC.Z));
                        ...
                        foreach (SupporSpecData SecData in ListCentalSuppoData[i].ListConcreteData)
                        {
                            ...
                            foreach (FaceData FaceLoc in SecData.ListfaceData)
                            {
                                ...
                                if (Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(GetVect3dFromVect3D(FaceLoc.FaceNormal), NEWWVC, new Vector3D(0, 0, 1)))) > 0 && Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(GetVect3dFromVect3D(FaceLoc.FaceNormal), NEWWVC, new Vector3D(0, 0, 1)))) < 89)
                                {
                                    FacestoProj.Add(FaceLoc);
                    ...
                    foreach (var part in SecPart2D)
                    {
                        if (part.Key == longver.SuppoId)
                        {
                            ...
                            if (EdgesSec.Count() > 0)
                            {
                                ...
                                //    Xoffset += 200;
                                //}
                                GENTOSLOC(AcadBlockTableRecord, AcadTransaction, AcadDatabase, Math.Max(topedge.P1.X, topedge.P2.X), topedge.P1.Y, MillimetersToMeters(longver.Boundingboxmax.Z).ToString(), Xoffset: Xoffset);
                    ...
                    foreach (var part in SecPart2D)
                    {
                        if (part.Key == longver.SuppoId)
                        {
                            ...
                            if (EdgesSec.Count() > 0)
                            {
                                ...
                                //    Xoffset += 200;
                                //}
                                ISMCTAG(AcadBlockTableRecord, AcadTransaction, AcadDatabase, new Point3d(leftedge.P1.X, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), "l", sectionsize, Xoffset);
                                AllParts.Add(Checkpart);
                                //if (PrimPart2D != null)
                                //{
                                //    PrimDimensionWithSec(longver, PrimPart2D, topedge, leftedge, i);
                    ...
                    if (part.Key == angsec.SuppoId)
                    {
                        ...
                        if (EdgesSec.Count() > 0)
                        {
                            ...
                            if (!slantside)
                            {
                                ...
                                CreateAlighDimen(AcadBlockTableRecord, AcadTransaction, new Point3d(Math.Min( touchbot.P1.X, touchbot.P2.X), touchbot.P1.Y, 0), new Point3d(Math.Min(tophor.P1.X, tophor.P2.X), tophor.P1.Y, 0), botsec2, -slantout.Length / 4);
                            }
                            else
                            {
                                GeometRi.Segment3d tophor = null;
        ...
        public void ConcretDimension(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, SupporSpecData concretepart, Dictionary<string, List<EntityData>> ConcretePart2D,int i)
        {
            ...
            if (botface.Count > 0)
            {
                ...
                {
                    TOSLOC(AcadBlockTableRecord, AcadTransaction, AcadDatabase, Math.Max(botedge.P1.X, botedge.P2.X), botedge.P1.Y, concretepart.Boundingboxmin.Z.ToString(), HTtype: "HPP(+)");
                        ...
                        if (prim.SuppoId == PrimID)
                        {
                            ...
                            try
                            {
                                if (prim.SupportName.ToLower().Contains("clamp") || prim.SupportName.ToLower().Contains("brac"))
                                {
                                    ...
                                    POS_OF_PRIMLEADER(AcadBlockTableRecord, AcadTransaction, AcadDatabase, prim, centerX, centerY + scalefactor, scalefactor, oddeven, prim_height, Yoffset, dirPrim);
                                }
        ...
        private void ISMCTAG(BlockTableRecord acadBlockTableRecord, Transaction AcadTransaction, Database acadDatabase, Point3d TagPos, [Optional] string ISMCTAGDir, double sectionsize = 100, double xOffset = 2000, double yOffset = 0)
        {
            ...
            {
                DrawLeader(acadBlockTableRecord, AcadTransaction, acadDatabase, "ISA " + sectionsize, new Point3d(TagPos.X, TagPos.Y + yOffset, 0), new Point3d(TagPos.X - xOffset, TagPos.Y + yOffset, 0));
            }
            else if (ISMCTAGDir == "LR" || ISMCTAGDir == "lr")
            {
                DrawLeader(acadBlockTableRecord, AcadTransaction, acadDatabase, "ISA " + sectionsize, new Point3d(TagPos.X, TagPos.Y + yOffset, 0), new Point3d(TagPos.X + xOffset, TagPos.Y + yOffset, 0));
