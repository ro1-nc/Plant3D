...
namespace Project1.Support2D
{
    public class SupportC
    {
        ...
        public void ProjectTo2D(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, ref double tracex, double boxlen, double boxht, ref double spaceY, Document Document2D, string SupType, [Optional] int i)
        {
            ...
            //                    if (Math.Abs(Math.Round(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, normal, new Vector3D(0, 0, 1))))) >= 0 && Math.Abs(Math.Round(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, normal, new Vector3D(0, 0, 1))))) < 10)
            //                    {
            ...
            //                    if (Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, NEWWVC, new Vector3D(0, 0, 1)))) >= 0 && Math.Abs(Calculate.ConvertRadiansToDegrees(Calculate.GetSignedRotation(FaceLoc.Normal, NEWWVC, new Vector3D(0, 0, 1)))) < 10)
            //                    {
        ...
        public void FJWELD(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, Dictionary<string, List<EntityData>> ConcretePart2D, int i)
        {
            ...
            if (topedge != null)
            {
                double centX = Math.Max(topedge.P1.X, topedge.P2.X);
                double centY = topedge.P2.Y;

                CopyAndModifyEntities(AcadBlockTableRecord, AcadTransaction, AcadDatabase, "LeaderBlock_Gen_R", centX, centY, topedge.Length / 2);
                            ...
                            if (EdgesSec.Count() > 0)
                            {
                                ...
                                if (Math.Abs(Math.Round(angle.XinDegree)).Equals(90) || Math.Abs(Math.Round(angle.XinDegree)).Equals(270))
                                {
                                    if (Math.Min(topedge.P1.X, topedge.P2.X) < minX)
                                    {
                                        ...
                                        {
                                            GENTOSLOC(AcadBlockTableRecord, AcadTransaction, AcadDatabase, Math.Min(topedge.P1.X, topedge.P2.X), topedge.P1.Y, MillimetersToMeters(secondary_sort.ElementAt(s).Boundingboxmax.Z).ToString(), position: "l", Xoffset: Xoffset);
                                            AllParts.Add(Checkpart);
                                            Xoffset = 1000;
