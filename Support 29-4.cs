    ...
    public class SupportC
    {
        ...
        public void DRS30(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, ref double tracex, double boxlen, double boxht, ref double spaceY, Document Document2D, string SupType, [Optional] int i)
        {
            ...
            if (ListCentalSuppoData[i].ListPrimarySuppo[0].SupportName.ToLower().Contains("nb"))
            {
                ...
                info[Defination.Prim_Radius] = radius;
                info[Defination.Prim_ht] = radius;
                CopyAndModifyEntities(AcadBlockTableRecord, AcadTransaction, AcadDatabase, "Front_Primary_Elevation", centerX + oofstlt * 0.34, centerY - oofstht, 801.5625);
                //height of prim from bottom
                try
                {
                    //    prim_height = MillimetersToMeters(Convert.ToDouble(prim_height)).ToString();
            ...
            if (ListCentalSuppoData[i].ListPrimarySuppo[0].SupportName.ToLower().Contains("clamp"))
            {
                ...
                CopyAndModifyEntities(AcadBlockTableRecord, AcadTransaction, AcadDatabase, "Front_Primary_Elevation", centerX, centerY, radius);
            ...
            //dimensioning
            CreateDimension(new Point3d(pointsinfo[Defination.SecTopRT].X + length, (pointsinfo[Defination.SecTopRT].Y + pointsinfo[Defination.SecTopRB].Y) / 2 + height / 2, 0), new Point3d(centerX + oofstlt * 0.34 + info[Defination.Prim_Radius], centerY - oofstht, 0), l_dist_frm_centre.ToString(), text_pos_rotation: -Math.PI / 2);
            CreateDimension(new Point3d(pointsinfo[Defination.SecTopRT].X + length, (pointsinfo[Defination.SecTopRT].Y + pointsinfo[Defination.SecTopRB].Y) / 2 - height / 2, 0), new Point3d(centerX + oofstlt * 0.34 + info[Defination.Prim_Radius], centerY - oofstht, 0), r_dist_frm_centre.ToString(), text_pos_rotation: -Math.PI / 2);
