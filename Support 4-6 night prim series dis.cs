...
namespace Project1.Support2D
{
    public class SupportC
    {
        ...
        public void GENTOSLOC(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, double centerX, double centerY, string TotalHt, string position = "r", string HTtype = "TOS EL.(+)", double Xoffset = 3500)
        {
            ...
            if (position.ToLower() == "l")
            {
                CreateMtextfunc(AcadBlockTableRecord, AcadTransaction, AcadDatabase, new Point3d(centerX - Xoffset, centerY + 100, 0), HTtype + TotalHt, 100);
        ...
        public void SecondaryDimension(BlockTableRecord AcadBlockTableRecord, Transaction AcadTransaction, Database AcadDatabase, IOrderedEnumerable<SupporSpecData> secondary_sort, Dictionary<string, List<EntityData>> SecPart2D, List<List<GeometRi.Segment3d>> AllParts, double dimminX, double dimmaxX, int i, Dictionary<string, List<EntityData>> PrimPart2D = null)
        {
                    ...
                    foreach (var part in SecPart2D)
                    {
                        if (part.Key == longver.SuppoId)
                        {
                            ...
                            if (EdgesSec.Count() > 0)
                            {
                                ...
                                CreateAlighDimen(AcadBlockTableRecord, AcadTransaction, new Point3d(rightedge.P1.X, Math.Max(rightedge.P1.Y, rightedge.P2.Y), 0), new Point3d(rightedge.P1.X, Math.Min(rightedge.P1.Y, rightedge.P2.Y), 0), l_dist_frm_centre.ToString(), -((dimmaxX - dimminX) / 2 + (topedge.Length / 2) + 200));
                                //}
                            }
                        }
                    }
                    ...
                    if (part.Key == angsec.SuppoId)
                    {
                        ...
                        if (EdgesSec.Count() > 0)
                        {
                            ...
                            if (!slantside)
                            {
                                ...
                                CreateAlighDimen(AcadBlockTableRecord, AcadTransaction, new Point3d(TopX, TopY, 0), new Point3d(BotX, BotY, 0), l_dist_frm_centre.ToString(), slantout.Length / 4);
                                //}
                            ...
                            if (EdgesSec.Count() > 0)
                            {
                                ...
                                if (Math.Abs(Math.Round(angle.XinDegree)).Equals(90) || Math.Abs(Math.Round(angle.XinDegree)).Equals(270))
                                {
                                    if (Math.Min(topedge.P1.X, topedge.P2.X) < minX)
                                    {
                                        ...
                                        {
                                            GENTOSLOC(AcadBlockTableRecord, AcadTransaction, AcadDatabase, Math.Min(topedge.P1.X, topedge.P2.X), topedge.P1.Y, MillimetersToMeters(secondary_sort.ElementAt(s).Boundingboxmax.Z).ToString(), "l", Xoffset: Xoffset);
                                            AllParts.Add(Checkpart);
                                            Xoffset = 1000;
            ...
            //for secondary of type angle 
            if (ListCentalSuppoData[i].ListSecondrySuppo.Count > 0)
            {
                for (int s = 0; s < secondary_sort.Count(); s++)
                {
                    angle = GetRotationFromVec(secondary_sort.ElementAt(s).Directionvec);
                    ...
                    if (secondary_sort.ElementAt(s).Size != null && (secondary_sort.ElementAt(s).Size.ToLower().Contains("angle") || secondary_sort.ElementAt(s).Size.ToLower().Contains("isa")))
                    {
                        if (secondary_sort.ElementAt(s).BoxData.Z < secondary_sort.ElementAt(s).BoxData.X || secondary_sort.ElementAt(s).BoxData.Z < secondary_sort.ElementAt(s).BoxData.Y)
                        {
                            foreach (var part in SecPart2D)
                            {
                                if (part.Key == secondary_sort.ElementAt(s).SuppoId)
                ...
                if (primtoucingsec != null)
                {
                    ...
                    if (Math.Abs(Math.Min(sectopedge.P1.X, sectopedge.P2.X) - ((topedge.P1.X + topedge.P2.X) / 2)) <= Math.Abs(Math.Max(sectopedge.P1.X, sectopedge.P2.X) - ((topedge.P1.X + topedge.P2.X) / 2)))
                    {
                        ...
                        if (ListCentalSuppoData[i].ListPrimarySuppo.Count > 0)
                        {
                            ...
                            {
                                CreateDimension(new Point3d(Math.Min(sectopedge.P1.X, sectopedge.P2.X), sectopedge.P1.Y, 0), new Point3d((topedge.P1.X + topedge.P2.X) / 2, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), final_distances_list.First().ToString(), topedge.P1.Y - (leftedge.P1.Y + leftedge.P2.Y) / 2, dimtextsize: (0.15 * topedge.Length) / 2);
                                for (int k = 1; k < primtoucingsec.Count(); k++)
                                {
                                    ...
                                    if (k % 2 == 0)
                                    {
                                        CreateDimension(new Point3d((prevtopedge.P1.X + prevtopedge.P2.X) / 2, (prevleftedge.P1.Y + prevleftedge.P2.Y) / 2, 0), new Point3d((topedge.P1.X + topedge.P2.X) / 2, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), final_distances_list[k].ToString(), topedge.P1.Y - (leftedge.P1.Y + leftedge.P2.Y) / 2, dimtextsize: (0.15 * topedge.Length) / 2);
                                    }
                                    else
                                    {
                                        CreateDimension(new Point3d((prevtopedge.P1.X + prevtopedge.P2.X) / 2, (prevleftedge.P1.Y + prevleftedge.P2.Y) / 2, 0), new Point3d((topedge.P1.X + topedge.P2.X) / 2, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), final_distances_list[k].ToString(), 1.5 * (topedge.P1.Y - (leftedge.P1.Y + leftedge.P2.Y) / 2), dimtextsize: (0.15 * topedge.Length) / 2);
                                    }
                                ...
                                CreateDimension(new Point3d(Math.Max(sectopedge.P1.X, sectopedge.P2.X), sectopedge.P1.Y, 0), new Point3d((topedge.P1.X + topedge.P2.X) / 2, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), last_dist.ToString(), topedge.P1.Y - (leftedge.P1.Y + leftedge.P2.Y) / 2, dimtextsize: (0.15 * topedge.Length) / 2);
                    ...
                    else
                    {
                        ...
                        if (ListCentalSuppoData[i].ListPrimarySuppo.Count > 0)
                        {
                            ...
                            if (sectopedge != null && topedge != null && leftedge != null)
                            {
                                ...
                                CreateDimension(new Point3d(Math.Min(sectopedge.P1.X, sectopedge.P2.X), sectopedge.P1.Y, 0), new Point3d((topedge.P1.X + topedge.P2.X) / 2, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), last_dist.ToString(), topedge.P1.Y - (leftedge.P1.Y + leftedge.P2.Y) / 2, dimtextsize: (0.15 * topedge.Length) / 2);
                                for (int k = 1; k < primtoucingsec.Count(); k++)
                                {
                                    foreach (var part in PrimPart2D)
                                    {
                                ...
                                CreateDimension(new Point3d(Math.Max(sectopedge.P1.X, sectopedge.P2.X), sectopedge.P1.Y, 0), new Point3d((topedge.P1.X + topedge.P2.X) / 2, (leftedge.P1.Y + leftedge.P2.Y) / 2, 0), final_distances_list.First().ToString(), topedge.P1.Y - (leftedge.P1.Y + leftedge.P2.Y) / 2, dimtextsize: (0.15 * topedge.Length) / 2);
